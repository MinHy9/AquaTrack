...
// 그래서 6초 이후로는 계속 맨앞에꺼 지우고 뒤에 추가하고 이런식으로 게속 그려지게 코드 구성됨
let subscription = null;

export default function drawChart(id) {
  if (subscription) {
  ...
  subscription = stompClient.subscribe(`/aquatrack/sensor/${id}`, (message) => {
    const data = JSON.parse(message.body);
    const recordedTime = new Date(data.recordedAt);
    const timeLabel = getTimeLabel(recordedTime);
